...
export const errorHandler = (res, stage, method, route) => err => {
  ...
  const stack = err.stack
    .split("\n")
    .slice(0, 5)
    .filter(line => !stackFilter.has(line.trim()))
    .join("\n");
  // tslint:disable-next-line
  debug(
    `  ${stage} =`.red,
...
export function registerMiddlewareErrorHandling(appRef) {
  appRef.use((err, _req, res, _next) =>
    errorHandler(res, "MIDDLEWARE-ERROR", "", "")(err)
  );
}
...
export default function registerApiNamespace(nsobj, appRef) {
  ...
  const functions = Object.getOwnPropertyNames(nsobj).filter(
    prop => typeof nsobj[prop] === "function"
  );

  return functions.forEach(funcName => {
    ...
    const middlewareNames = middleware
      .map(middlewareFunc => middlewareFunc.name)
      .join("->");
    let route = "/api";
    ...
    debug(
      `${method.toLocaleUpperCase().padEnd(4).green} ${route.padEnd(
        25,
        "_"
      )} {fn: ${funcName.slice(0, 14).padEnd(14)} MD: ${middlewareNames ||
        "<none>"}}`
    ); // eslint-disable-line
